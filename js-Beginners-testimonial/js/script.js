(function () {
  ...
  function customerConstructor(img, name, text) {
    this.img = img;
    this.name = name;
    this.text = text;
  }
  ...
  function createCustomer(img, name, text) {
    ...
    let customer = new customerConstructor(fullimg, name, text);
  ...
  slideButtons.forEach(function (button) {
    button.addEventListener("click", function (e) {
      if (e.target.closest("a").classList.contains("prevBtn")) {
        index--;
        if (index < 0) {
      ...
      if (e.target.closest("a").classList.contains("nextBtn")) {
        index++;
